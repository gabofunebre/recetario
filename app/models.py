...
class Receta(db.Model):
    ...
    descripcion = db.Column(db.Text, nullable=True)  # <-- Ahora opcional
...
class Ingrediente(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    nombre = db.Column(db.String(100), nullable=False)
    cantidad = db.Column(db.String(50))  # Cantidad con unidad (por ejemplo: 100g, 2 tazas)

    # Clave foránea a la receta correspondiente
    receta_id = db.Column(db.Integer, db.ForeignKey('recetas.id'), nullable=False)

    def __repr__(self):
        return f"<Ingrediente {self.nombre} ({self.cantidad})>"
